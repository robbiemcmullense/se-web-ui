      ...
      <Host class={!isHexColor ? `color-${this.secolor}` : ''}>
        <div class={["radial-wrapper", this.size].join(' ')}>
          <svg class="radial" height={svgSize} width={svgSize} >
            <circle cx={circleDimensions} cy={circleDimensions} r={circleRadius} stroke="currentColor" stroke-width="8" fill="transparent" class="back-circle"></circle>
            <circle cx={circleDimensions} cy={circleDimensions} r={circleRadius} stroke={isHexColor ? this.secolor : 'currentColor'} stroke-width="8" fill="transparent" style={{ strokeDashoffset: String(offset) }}></circle>
            Sorry, your browser does not support inline SVG.
          </svg>
          <svg height={svgSize} width={svgSize} class="text">
            <text>
              {this.value ? <tspan class="value" x="50%" y="50%" text-anchor="middle" fill="currentColor">{this.value}</tspan> : ''}
              <tspan class="label" x="50%" y={this.value ? "70%" : "53%"} text-anchor="middle" fill="currentColor">{this.label}</tspan>
            </text>
          </svg>
        </div>
      </ Host>
