...
describe('BannerItemComponent', () => {
  ...
  it('should render an image container element', async () => {
    const imageContainer = await page.find(
      'se-banner-item >>> .image-container'
    );
    expect(imageContainer).toBeTruthy();
  });

  it('should render a div element with the banner section wrapper class', async () => {
    const bannerSection = await page.find(
      'se-banner-item >>> .banner-section-wrapper'
    );
...
describe('BannerItemComponent with block and button components', () => {
  ...
  beforeEach(async () => {
    ...
    await page.setContent(
      '<se-banner-item>><se-block id="block"><se-button id="button"></se-button></se-block></se-banner-item>'
    );
  ...
  it('adds a banner-child class to the host block element and the block-bg-alternative color to its internal markup', async () => {
    ...
    expect(blockElement.shadowRoot.querySelector('.block-body')).toHaveClass(
  ...
  it('adds a banner-child class to the host button element', async () => {
    ...
    expect(buttonElement).toHaveClass('banner-child');
  });
});
