 ...
 describe('ListGroupComponent', () => {
  ...
  it('renders', async() => {
	...
	it('renders with an se-list-item element by default, passing its text property', async() => {
		await page.setContent('<se-list-group text="Item Group"></se-list-group>');
		const element = await page.find('se-list-group >>> se-list-item');
		expect(element).toEqualHtml('<se-list-item class="classic hydrated" text="Item Group"></se-list-item>');
	});
	...
	it('renders with 3 child list items', async() => {
		...
		await page.setContent('<se-list-group text="Item Group"><se-list-item text="one"></se-list-item><se-list-item text="two"></se-list-item><se-list-item text="three"></se-list-item></se-list-group>');
