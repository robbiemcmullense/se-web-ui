import { newE2EPage } from '@stencil/core/testing';
 ...
 describe('ListGroupComponent', () => {
  ...
  it('renders', async() => {	
	...
	it('renders with an se-list-item element by default, passing its item-title property', async() => {	
		await page.setContent('<se-list-group item-title="Item Group"></se-list-group>');
		const element = await page.find('se-list-group >>> se-list-item');
		expect(element).toEqualHtml('<se-list-item class="classic hydrated" item-title="Item Group"></se-list-item>');
	});
	...
	it('renders with 3 child list items', async() => {
		...
		await page.setContent('<se-list-group item-title="Item Group"><se-list-item item-title="one"></se-list-item><se-list-item item-title="two"></se-list-item><se-list-item item-title="three"></se-list-item></se-list-group>');
