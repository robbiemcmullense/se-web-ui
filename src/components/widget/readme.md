...
| `enlarged` | `enlarged` | When on Grid mode, define if the widget should be a 2/2 instead of a small 1/1 grid item.                                                                                                   | `boolean`                     | `false`         |
...
| `mode`     | `mode`     | Define the mode of a widget. The default widget gives a small padding of the widget. `fill` will remove any spacing. `card` will create a card look and fell with shadow and rounded corner | `"card" \| "fill"`            | `undefined`     |
