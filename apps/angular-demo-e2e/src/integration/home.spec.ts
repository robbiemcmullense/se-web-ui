describe('Angular Home Page', () => {
  ...
  it('should trigger a dialog confirm component when clicking on the dialog simple button', () => {
    const dialogComponent = cy.get('se-dialog');
    dialogComponent.should('not.be.visible');
    const dialogButton = cy.shadowGet('se-button').contains('Open confirm');
  ...
  it('should trigger a dialog table component when clicking on the dialog simple button', () => {
    const dialogComponent = cy.get('se-dialog');
    dialogComponent.should('not.be.visible');
    const dialogButton = cy.shadowGet('se-button').contains('Open table');
    dialogButton.should('exist');
    dialogButton.click();
    dialogComponent.should('be.visible');
  });
  ...
  it('should remove the dialog component from the DOM when clicking the Ok button', () => {
    cy.shadowGet('se-button').contains('Submit').shadowClick();
    cy.get('se-dialog').should('not.exist');
  });
